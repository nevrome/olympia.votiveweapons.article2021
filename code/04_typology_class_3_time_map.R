...
artefact_timeseries_df <-  aoristAAR::aorist(
  artefacts,
  split_vars = c("find_area", "typology_class_3"),
  from = "dating_typology_start",
  to = "dating_typology_end",
  method = "number"
)

seqofinterest <- seq(-1000, -500, 100)

artefact_number_timesteps <- artefact_timeseries_df %>%
  dplyr::filter(
...
artefact_number_timesteps_sf <- artefact_number_timesteps %>%
  dplyr::right_join(
    expand.grid(
      ...
      typology_class_3 = unique(artefact_number_timesteps$typology_class_3),
      stringsAsFactors = F
    ),
    by = c("date", "find_area", "typology_class_3")
  ) %>%
  dplyr::left_join(
    site_areas, 
...
p <- ggplot() +
  ...
  facet_grid(
    rows = dplyr::vars(typology_class_3), cols = dplyr::vars(date)
  ) +
  theme_bw() +
  theme(
...
ggsave(
  filename = "04_typology_class_3_time_map.png",
  plot = p,
  device = "png",
  path = "plots/",
  width = 400,
